...
const ProductPage = ({ product, relatedProducts, technicalSpecifications }) => {
  ...
  return (
    <>
      ...
      {technicalSpecifications && (<AccordionModule items={technicalSpecifications} accentText="Technical Specifications" />)}
      {relatedProducts.length > 0 && (<RelatedProducts title="More in our Terrazzo range" products={relatedProducts} />)}
    </>
  );
};
